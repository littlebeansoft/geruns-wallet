...
function show_data(page){
	...
	.done(function(data){
		...
		$.each(obj,function(i){	
			if(i!=='pagination'){
				if(ty==''){
					da+='<tr class="table-success"><th colspan="6" >'+obj[i].menu+'</th><tr>';
					ty=obj[i].menu_id;
					}else{
					if(ty!==obj[i].menu_id){
						da+='<tr class="table-success"><th colspan="6">'+obj[i].menu+'</th><tr>';
				...
				da+='<td>'+obj[i].menu2_url+'</td>';
				da+='<td><i class="fa '+obj[i].menu2_icon+'"></i> : '+obj[i].menu2_icon+'</td>';
				da+='<td>'+obj[i].menu2_file+'</td>';			
				...
				da+='Edit';
				da+='</a>';
				da+='<a style="cursor:pointer" onClick="delete_data('+"'"+obj[i].id+"','"+obj[i].menu_id+"'"+')" class="dropdown-item">';
				da+='<i class="fa fa-trash-alt text-danger-m1 mr-1 p-2 w-4"></i>';
				da+='Delete';
				da+='</a>';
				da+='<a style="cursor:pointer" onClick="sort_data('+"'"+obj[i].id+"','"+obj[i].title+"','"+i+"','"+obj[i].menu_id+"'"+')" class="dropdown-item" data-toggle="modal" data-target="#dialog-sort">';
...
function chk_menu_url(){
	if($('#menu_id').val()!==''){
		let param = new FormData();
		param.append('menu2_url',$('#menu2_url').val());
		param.append('menu_id',$('#menu_id').val());
...
//ย้ายตำแหน่งข้อมูล
function sort_data(id,label,order,pid){
...
function delete_data(did,pid){
	...
	ms+='<button type="button" data-dismiss="toast" class="btn px-4 btn-danger" onclick="confirm_delete('+"'"+did+"','"+pid+"'"+')">ยืนยันการลบข้อมูล</button>';
...
function confirm_delete(did,pid){
	let param=new FormData();
	param.append('id',did);
...
$(function(){
    ...
    $('.select2').select2({
	    ...
	    // duallist box
		var demo1 = $('select[name="menu2_id[]"]').bootstrapDualListbox({infoTextFiltered: '<span class="badge badge-purple badge-lg">Filtered</span>'});
	...
	$('#menu_id').change(function(){testcheck($('#menu_id').val());});
	...
	//เช็ค menu2_url ซ้ำ
	$('#menu2_url').blur(function(){ chk_menu_url();});	
	$('#menu_id').change(function(){ chk_menu_url();});	
	
	//คลิกเลือกประเภทแล้ว disable/enable input 
	$('#menu_type').change(function(){disable_input('menu_type','2','menu2_file');});	
	
	//คลิกปุ่ม reset แล้วไม่ enable menu2_file
	$('#btn-reset').click(function(){ $('#menu2_file').attr('disabled',false);});	
	...
	$('#frm-main').validate({
		...
		messages: {
			menu_id: {
				required: " กรุณาเลือกเมนูหลักด้วยครับ",
			},
			menu2_url: {
